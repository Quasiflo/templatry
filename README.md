...
- `generate` writes every enabled template on every run (no change detection shortcuts). Files from previously generated destinations that are no longer planned are pruned automatically: `templatry.manifest.json` (written alongside the project config, expected to be gitignored) tracks what was written, and only files whose content still matches the recorded hash are deleted — anything else is kept with a warning. `--dry-run` prints planned writes (plus `would delete …` for stale files) instead; `--check` prints differing paths without writing and exits `2` (for CI and pre-commit hooks, stale files included); `--offline` fails on cache misses instead of fetching. `--watch` reruns on file changes (see Watch Mode). `--check`, `--dry-run`, and `--watch` are mutually exclusive.
