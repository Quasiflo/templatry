...
Rust CLI + library, Milestones 0–5 done. `validate` resolves all four source kinds (remotes fetched through the content-addressed cache); `generate` (with `--check`/`--dry-run`/`--watch`), `validate`, and `cache clear` work; `back_propagate` templates fold generated edits into overrides with safety replays.
...
- Pinned Dependencies; `deny.toml` enforces advisories/bans/licenses/sources.
